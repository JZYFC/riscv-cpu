...
| RV32M 除法（DIV/DIVU/REM/REMU） | 完整实现（32 周期恢复式迭代除法器，符合 RISC-V 规范边界情况） |
| RV32F 浮点（基本算术） | ibex_fpu 已接入，部分指令可执行 |
| CSR 指令（CSRRW/CSRRS/CSRRC 及立即数变体） | 实现，CSR 文件 4096 项 |
| FENCE / ECALL / EBREAK | 识别并作为 SYSTEM 指令处理 |
...
│                           流水线各阶段                                   │
│                                                                         │
│  ICache ──→ IF ──→ InstructionBuffer ──→ PreDecode ──→ RegRename        │
│                                                          │ (含 ROB)     │
│                                                          ▼              │
│                                                      PostDecode         │
│                                                          │              │
│                                                          ▼              │
│  PhysicalRegFile ←──── IssueBuffer(保留站) ──→ 执行单元               │
│         │                    │                     │                    │
│         │                    ▼                     │                    │
│         │              IntALU ×2                   │                    │
│         │              MulDiv ×2(单周期乘)          │                    │
...
│         │              System/CSR ×2               │                    │
│         │              ibex_fpu ×2                 │                    │
│         │              LSU ×1 ──→ DCache ──→ MemArbiter ──→ MainMemory │
│         │                                           │                    │
│         └──────────── 写回（WB）←──────────────────┘                    │
│                          │                                              │
│                          ▼                                              │
